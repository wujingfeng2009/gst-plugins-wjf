...
#define TYPE_READER_SKIP_STRING(bits) \
gboolean \
 ...
 * Since: 0.10.24
 */
TYPE_READER_SKIP_STRING (8);

/**
 * type_reader_skip_string_utf16:
 * @reader: a #TypeReader instance
 *
 * Skips a NUL-terminated UTF-16 string in the #TypeReader instance,
 ...
 */
gboolean
type_reader_peek_string_utf8 (const TypeReader * reader,
    const gchar ** str)
{
  g_return_val_if_fail (reader != NULL, FALSE);
  g_return_val_if_fail (str != NULL, FALSE);

  if (type_reader_scan_string_utf8 (reader) > 0) {
 ...
 */
gboolean
type_reader_get_string_utf8 (TypeReader * reader, const gchar ** str)
{
  guint size;                   /* size in bytes including the terminator */
  ...
  size = type_reader_scan_string_utf8 (reader);
  if (size == 0) {
    *str = NULL;
    return FALSE;
  }
...
#define TYPE_READER_DUP_STRING(bits,type) \
gboolean \
 ...
 * Since: 0.10.24
 */
TYPE_READER_DUP_STRING (8, gchar);
 ...
 * Since: 0.10.24
 */
TYPE_READER_DUP_STRING (16, guint16);
 ...
 * Since: 0.10.24
 */
TYPE_READER_DUP_STRING (32, guint32);
